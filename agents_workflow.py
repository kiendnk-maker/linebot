...
async def run_debate(user_id: str, question: str, rounds: int = 2) -> str:
    """
    /debate — Two LLMs argue back and forth, then a judge synthesizes.
    Model A (Large): Advocate — builds the case
    Model B (Magistral): Critic — finds flaws and counter-argues
    Judge (Large): Synthesizes both sides into final answer
    """
    import main

    model_a = "mistral-large-latest"
    model_b = "magistral-medium-latest"
    ...
    for i in range(rounds):
        ...
        if i == 0:
            a_prompt = (
                f"Bạn là chuyên gia phân tích (Advocate). "
                f"Đưa ra quan điểm rõ ràng, có luận cứ cho câu hỏi sau:\n\n"
                f"{question}"
            )
        else:
            a_prompt = (
                f"Đối phương (Critic) phản biện:\n{b_says}\n\n"
                f"Hãy bảo vệ hoặc điều chỉnh quan điểm của bạn. "
                f"Thừa nhận điểm đúng của đối phương nếu có."
            )

        a_says = await main.call_mistral_text(
            history=[{"role": "user", "content": a_prompt}],
            model_id=model_a,
            model_key="large",
            user_id=user_id,
        )
        a_history.append(a_says)
        debate_log.append(f"🔵 Advocate (R{round_num}):\n{a_says}")

        # ── Model B: Critic ──
        b_prompt = (
            f"Bạn là chuyên gia phản biện (Critic). "
            f"Phân tích quan điểm sau, tìm điểm yếu, thiếu sót, hoặc góc nhìn bị bỏ qua. "
            f"Đưa ra phản biện có logic.\n\n"
            f"Quan điểm của Advocate:\n{a_says}"
        )

        b_says = await main.call_mistral_text(
            history=[{"role": "user", "content": b_prompt}],
            model_id=model_b,
            model_key="reason",
            user_id=user_id,
        )
        b_history.append(b_says)
        debate_log.append(f"🔴 Critic (R{round_num}):\n{b_says}")

    # ── Judge: Synthesize ──
    judge_prompt = (
        f"Bạn là trọng tài trung lập. Đọc cuộc tranh luận sau và đưa ra "
        f"kết luận cuối cùng, tổng hợp điểm mạnh của cả hai bên.\n\n"
        f"Câu hỏi gốc: {question}\n\n"
    )
    for i in range(rounds):
        judge_prompt += f"--- Vòng {i+1} ---\n"
        judge_prompt += f"Advocate: {a_history[i][:500]}\n"
        judge_prompt += f"Critic: {b_history[i][:500]}\n\n"
    judge_prompt += "Kết luận cuối cùng (tổng hợp cả hai bên):"

    verdict = await main.call_mistral_text(
        history=[{"role": "user", "content": judge_prompt}],
        model_id=model_a,
        model_key="large",
        user_id=user_id,
    )
    ...
    # ── Format output ──
    separator = "─" * 20
    output = f"⚔️ [DEBATE MODE — {rounds} vòng]\n{separator}\n"
