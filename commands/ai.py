...
async def handle_ai_command(user_id: str, cmd: str, arg: str) -> str | None:
    ...
    if cmd == "p":
        if not arg:
            return "⚠️ Gõ /p <câu hỏi> để dùng Pro mode"
        return await run_pro_workflow(user_id, arg)

    if cmd == "ag":
        if not arg:
            return "⚠️ Gõ /ag <task> để dùng Agent mode"
        return await run_agentic_loop(user_id, arg)

    if cmd == "co":
        if not arg:
            return "⚠️ Gõ /co <yêu cầu> để dùng Coder mode"
        return await run_multi_agent_workflow(user_id, arg)

    if cmd == "d":
        if not arg:
            return (
                "⚔️ Debate mode — 2 AI tranh luận\n"
                "Ví dụ: /d Nên học Thạc sĩ hay đi làm?"
            )
        ...
        try:
            ...
            return "⚠️ Debate quá 25 giây, rút gọn câu hỏi nhé!"
