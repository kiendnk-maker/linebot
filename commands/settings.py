...
async def handle_settings_command(user_id: str, cmd: str, arg: str) -> str | None:
    ...
    if cmd == "m":
        if not arg:
            return "🔢 Gõ /m <số> (1-9) để chọn model nhanh\n(1-5: Mistral, 6-9: Groq)"
        try:
            choice = int(arg)
            model_map = {
                1: "mistral_small",
            ...
            if choice in model_map:
                ...
                # Check if this model requires Mistral API
                if MODEL_REGISTRY[model_key].get("requires_mistral_api"):
                    return f"✅ Đã chọn {MODEL_REGISTRY[model_key]['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
                else:
                    return f"✅ Đã chuyển sang {MODEL_REGISTRY[model_key]['display']}"
            else:
                return "❌ Chọn số 1-9 thôi nhé!"
        except ValueError:
            return "❌ Gõ /m <số> (1-9)"
    
    # Mistral AI model shortcuts (with helpful warnings)
    if cmd == "mistral_small":
        await set_user_model(user_id, "mistral_small")
        return f"✅ Đã chọn {MODEL_REGISTRY['mistral_small']['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
    
    if cmd == "mistral_medium":
        await set_user_model(user_id, "mistral_medium")
        return f"✅ Đã chọn {MODEL_REGISTRY['mistral_medium']['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
    
    if cmd == "mistral_large":
        await set_user_model(user_id, "mistral_large")
        return f"✅ Đã chọn {MODEL_REGISTRY['mistral_large']['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
    
    if cmd == "codestral":
        await set_user_model(user_id, "codestral")
        return f"✅ Đã chọn {MODEL_REGISTRY['codestral']['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
    
    if cmd == "pixtral":
        await set_user_model(user_id, "pixtral")
        return f"✅ Đã chọn {MODEL_REGISTRY['pixtral']['display']}\n⚠️ Yêu cầu API Mistral - sử dụng /export nếu cần lưu câu hỏi"
...
def _models_list_text() -> str:
    return (
        ...
        "4💻 Codestral | 5👁 Pixtral (yêu cầu API)\n"
        "6⚡ Llama 8B | 7🦙 Llama 70B | 8🧠 Qwen3\n"
        "9⚡ Groq Fast (luôn sẵn sàng)\n"
        "\n"
        "🤖 AI MODES\n"
