...
# ── Available models (Updated for 2026 stability) ──────────────────────────────
...
# ── Groq call (FIXED: Proxies Error) ──────────────────────────────────────────
def call_groq(history: list[dict], model: str) -> str:
    """
    Sửa lỗi 'unexpected keyword argument proxies' bằng cách dùng httpx.Client 
    không có proxies cấu hình sẵn hoặc khởi tạo sạch.
    """
    # Khởi tạo http_client sạch để tránh lỗi proxies từ biến môi trường hệ thống
...
async def lifespan(app: FastAPI):
    await init_db()
    print("✅ Database & Bot Groq哥哥 ready!")
    yield
...
async def handle_command(user_id: str, text: str) -> str | None:
    ...
    if cmd == "/help":
        ...
        return (
            ...
            "/clear - Xoá nhớ\n"
            "/model <tên> - Đổi model\n"
            "/model list - Xem danh sách\n"
            f"\nModel hiện có:\n{model_list}"
        )

    if cmd == "/model list":
        return "📋 Model khả dụng: " + ", ".join(AVAILABLE_MODELS.keys())

    if cmd.startswith("/model "):
        arg = text.strip()[7:].strip().lower()
        target = AVAILABLE_MODELS.get(arg)
        if target:
            await set_model(user_id, target)
            return f"✅ Đã chuyển sang model: {arg}"
        return f"❌ Không tìm thấy model: {arg}"
...
async def process_message(event: MessageEvent) -> None:
    ...
    async with AsyncApiClient(line_config) as api_client:
        ...
        try:
            ...
            reply_text = f"⚠️ Có lỗi xảy ra: {str(e)[:100]}"
...
@app.get("/")
async def health():
    return {"status": "running", "bot": "Groq哥哥"}